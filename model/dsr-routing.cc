...
//extern double DsrOptionResponceRep::vector[];
//double DsrOptionResponceRep::vector[DsrOptionResponceRep::SIZE_VECTOR];

void DsrRouting::Start ()
{
  NS_LOG_FUNCTION (this << "Start DSR Routing protocol");
  std::cout<< "Start DSR Routing protocol"<<std::endl;
  ...
  if (dsrRouting.GetMessageType () == 2 && ourAddress == m_mainAddress)
    {
      ...
      //std::cout<<"The previous node " << previousHop<<std::endl;
...
{
  std::cout<<"CheckSendBuffer"<<std::endl;
...
// void DsrRouting::SendResponceRep(Ptr<Packet>p){
//   // uint8_t salvage = 0;
//   //  DsrOptionResponceRep responce;
//   //     responce.SetErrorType (4);               // The error type 3 means Option not supported
//   //     responce.SetErrorSrc (m_mainAddress);       // The error source address is our own address
//   //     responce.SetUnsupported (optionType);       // The unsupported option type number
//   //     responce.SetErrorDst (src);              // Error destination address is the destination of the data packet
//   //     responce.SetSalvage (salvage);           // Set the value about whether to salvage a packet or not
//   //     SendError (responce, 0, protocol); // Send the error packet
//       /*
...
{
  //std::cout<<"PromiscReceive"<<std::endl;
  ...
  if (protocol == Ipv4L3Protocol::PROT_NUMBER && packetType == NetDevice::PACKET_OTHERHOST)
    {
      ...
      if (ipv4Header.GetProtocol () == DsrRouting::PROT_NUMBER)
        {
          //std::cout<<"Receive. FROM:"<< ipv4Header.GetSource() <<" TO:"<< ipv4Header.GetDestination() <<std::endl;
...
DsrRouting::PacketNewRoute (Ptr<Packet> packet,
                            ...
                            uint8_t protocol)
{
  std::cout<<"PacketNewRoute"<<std::endl;
  ...
  if (!findRoute)
    {
      std::cout<<"PacketNewRoute IF"<<std::endl;
      NS_LOG_INFO (Simulator::Now ().GetSeconds ()
          ...
          if ((m_addressReqTimer.find (destination) == m_addressReqTimer.end ()) && (m_nonPropReqTimer.find (destination) == m_nonPropReqTimer.end ()))
            {
               ...
               */
              std::cout<<"SendInitialRequest"<<std::endl;
    ...
    {
       std::cout<<"PacketNewRoute ELSE"<<std::endl;
      Ptr<Packet> cleanP = packet->Copy ();
      DsrRoutingHeader dsrRoutingHeader;
      dsrRoutingHeader.SetNextHeader (protocol);
      dsrRoutingHeader.SetMessageType (2);
...
{
 std::cout<<"SendUnreachError"<<std::endl; 
...
DsrRouting::ForwardErrPacket (DsrOptionRerrUnreachHeader &rerr,
                              ...
                              Ptr<Ipv4Route> route)
{
  std::cout<<"ForwardErrPacket"<<std::endl;
 ...
 void DsrRouting::printMap()
{
  ...
  for (std::map<Ipv4Address, std::pair<int, int> >::const_iterator it = this->statisticConteiner.begin();
       it != this->statisticConteiner.end(); ++it)
  {
                  ...
                  Ptr<Ipv4Route> route)
{
  ...
  if (protocol == 1)
    {
      std::cout<<"Drop packet. Not handling ICMP packet for now"<<std::endl;
...
DsrRouting::SalvagePacket (Ptr<const Packet> packet, Ipv4Address source, Ipv4Address dst, uint8_t protocol)
{
  ...
  if (findRoute && (salvage < m_maxSalvageCount))
    {
      std::cout<<"SalvagePacket"<<std::endl;
...
DsrRouting::SendErrorRequest (DsrOptionRerrUnreachHeader &rerr, uint8_t protocol)
{
  ...
  if (m_routeCache->LookupRoute (dst, toDst))
  ...
  else
    {
      ...
      //std::cout<<"SendErrorRequst ELSE"<<std::endl;
      Ptr<Packet> packet = Create<Packet> ();
...
void
DsrRouting::SendMyErrorRequest (DsrOptionResponceRep &rerr, uint8_t protocol)
{
  NS_LOG_FUNCTION (this << (uint32_t)protocol);
  NS_ASSERT_MSG (!m_downTarget.IsNull (), "Error, DsrRouting cannot send downward");
  uint8_t salvage = rerr.GetSalvage ();
...
void
DsrRouting::SendMyResponce  (Ipv4Address unreachNode, Ipv4Address destination, Ipv4Address originalDst, uint8_t salvage, uint8_t protocol)
{
  ...
  DsrOptionResponceRep rerrUnreachHeader;
  rerrUnreachHeader.SetErrorType (4);
  rerrUnreachHeader.SetErrorSrc (m_mainAddress);
  rerrUnreachHeader.SetUnreachNode (unreachNode);
  rerrUnreachHeader.SetErrorDst (destination);
  rerrUnreachHeader.SetOriginalDst (originalDst);
...
void
DsrRouting::SendAckRep  (Ipv4Address unreachNode, Ipv4Address destination, Ipv4Address originalDst, uint8_t salvage, uint8_t protocol)
{
  ...
  DsrOptionAckRep rerrUnreachHeader;
  rerrUnreachHeader.SetErrorType (5);
  rerrUnreachHeader.SetErrorSrc (m_mainAddress);
  rerrUnreachHeader.SetAckRepNode (unreachNode);
  rerrUnreachHeader.SetErrorDst (destination);
  rerrUnreachHeader.SetOriginalDst (originalDst);
  rerrUnreachHeader.SetSalvage (salvage);                       // Set the value about whether to salvage a packet or not
                     ...
                     Ptr<Ipv4Interface> incomingInterface)
{
  ...
  if (optionType == 1)        // This is the request option
  ...
  else if (optionType == 3)       // This is a route error header
    {
      ...
      uint32_t size = tempPacket->GetSize ();
      uint8_t *data = new uint8_t[size];
      ...
      if(errorType==(uint8_t)5){ //5 - RECEIVE ACK_REP

        // std::cout<<"CHECK"<<std::endl;
        //std::cout<<"Get ACK_REP"<<std::endl;
        Ptr<Packet> p = tempPacket->Copy ();
        DsrOptionAckRep rrep;
        p->RemoveHeader (rrep);
  ...
  else
    {
      ...
      if(errorType==(uint8_t)4){ //4 - RECEIVE RESPONCE
        ...
        //rrep.GetRepVector();
        //std::map<Ipv4Address, double> gotVector=rrep.GetRepVector();
        //std::cout<<"Serialize size: "<<rrep.GetSerializedSize()<<std::endl;

          // std::cout<<"Node: "<<m_mainAddress<<std::endl;
          // for (std::map<Ipv4Address, double >::const_iterator it = gotVector.begin();
          //     it != gotVector.end(); ++it)
          // {
          //   std::cout << it->first << " VectorSend rep:" << it->second<< "\n";
          // }
          // std::cout<<"---------------------"<<std::endl;
