...
NS_OBJECT_ENSURE_REGISTERED (DsrOptionResponceRep);




TypeId DsrOptionResponceRep::GetTypeId ()
{
  static TypeId tid = TypeId ("ns3::dsr::DsrOptionResponceRep")
    .AddConstructor<DsrOptionResponceRep> ()
    .SetParent<DsrOptionRerrHeader> ()
    .SetGroupName ("Dsr")
  ;
  return tid;
...
TypeId DsrOptionResponceRep::GetInstanceTypeId () const
{
  return GetTypeId ();
}

DsrOptionResponceRep::DsrOptionResponceRep ()
  :
    m_salvage (0)
{
...
void DsrOptionResponceRep::SetSalvage (uint8_t salvage)
{
  m_salvage = salvage;
}

void DsrOptionResponceRep::SetRepVector (std::map<Ipv4Address, std::pair<int,int> > m){
...
uint8_t DsrOptionResponceRep::GetSalvage () const
{
  return m_salvage;
}

void DsrOptionResponceRep::SetErrorSrc (Ipv4Address errorSrcAddress)
{
  m_errorSrcAddress = errorSrcAddress;
}

Ipv4Address DsrOptionResponceRep::GetErrorSrc () const
{
  return m_errorSrcAddress;
}

void DsrOptionResponceRep::SetErrorDst (Ipv4Address errorDstAddress)
{
  m_errorDstAddress = errorDstAddress;
}

Ipv4Address DsrOptionResponceRep::GetErrorDst () const
{
  return m_errorDstAddress;
}

void DsrOptionResponceRep::SetUnreachNode (Ipv4Address unreachNode)
{
  m_unreachNode = unreachNode;
}

Ipv4Address DsrOptionResponceRep::GetUnreachNode () const
{
  return m_unreachNode;
}

void DsrOptionResponceRep::SetOriginalDst (Ipv4Address originalDst)
{
  m_originalDst = originalDst;
}

Ipv4Address DsrOptionResponceRep::GetOriginalDst () const
{
  return m_originalDst;
}

void DsrOptionResponceRep::Print (std::ostream &os) const
{
  os << "( type = " << (uint32_t)GetType () << " length = " << (uint32_t)GetLength ()
     << " errorType = " << (uint32_t)m_errorType << " salvage = " << (uint32_t)m_salvage
     << " error source = " << m_errorSrcAddress << " error dst = " << m_errorDstAddress
...
uint32_t DsrOptionResponceRep::GetSerializedSize () const
{
  //size_t size=vectorMap.size()+1;
...
void DsrOptionResponceRep::Serialize (Buffer::Iterator start) const
{
  Buffer::Iterator i = start;

  //SetLength(20+6+vectorMap.size()*6);
    
  i.WriteU8 (GetType ());
  //i.WriteU8 (20+6+vectorMap.size()*6);
  i.WriteU8(GetLength());
  //std::cout<<"Lenght in Serialize: "<<(int)GetLength()<<std::endl;
  i.WriteU8 (GetErrorType ());
  i.WriteU8 (m_salvage);
  WriteTo (i, m_errorSrcAddress);
  WriteTo (i, m_errorDstAddress);
...
uint32_t DsrOptionResponceRep::Deserialize (Buffer::Iterator start)
{
  Buffer::Iterator i = start;

  SetType (i.ReadU8 ());
  uint8_t lengthData=i.ReadU8();
  //SetLength (i.ReadU8 ();
  SetLength (lengthData);
  //std::cout<<"Lenght in Deserialize: "<<(int)GetLength()<<std::endl;
  SetErrorType (i.ReadU8 ());
  m_salvage = i.ReadU8 ();
  ReadFrom (i, m_errorSrcAddress);
  ReadFrom (i, m_errorDstAddress);
...
DsrOptionHeader::Alignment DsrOptionResponceRep::GetAlignment () const
{
  Alignment retVal = { 4, 0 };
  return retVal;
}
